...
public class AuthControllerTest {
    ...
    void testPostRefresh_WithInvalidToken_ShouldReturnForbidden() throws Exception {
        when(refreshTokenService.validateRefreshToken("invalid-refresh-token"))
                .thenThrow(new TokenRefreshException("Refresh token inválido, expirado, revogado ou inexistente"));
        ...
        mockMvc.perform(post("/auth/refresh")
                ...
                .content(refreshRequest))
                .andExpect(status().isUnauthorized())
                .andExpect(jsonPath("$.message").value("Refresh token inválido, expirado, revogado ou inexistente"));
    }
}
